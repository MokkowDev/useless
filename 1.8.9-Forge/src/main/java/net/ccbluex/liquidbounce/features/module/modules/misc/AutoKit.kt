...
@ModuleInfo(name = "AutoKit", category = ModuleCategory.MISC, description = "skid")
class AutoKit : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (clickStage == 2) {
            ...
            if (timeoutTimer.hasTimePassed(40)) {
                ...
                LiquidBounce.hud.addNotification(Notification(this.name,"Kit checker timed out. Please use the right kit name.", NotifyType.ERROR,3500))
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is C0DPacketCloseWindow && editMode.get()) {
            editMode.set(false)
            LiquidBounce.hud.addNotification(Notification(this.name,"Edit mode aborted.", NotifyType.INFO,3500))
        ...
        if (packet is S02PacketChat) {
            ...
            if (text.contains("kit has been selected", true)) {
                if (editMode.get()) {
                    ...
                    clickStage = 0
                    listening = false
                    LiquidBounce.hud.addNotification(Notification(this.name,"Successfully detected and added $kitName kit.", NotifyType.INFO,3500))
                    debug("finished detecting kit")
                    return
                } else {
                    listening = false
                    event.cancelEvent()
                    LiquidBounce.hud.addNotification(Notification(this.name,"Successfully selected ${kitNameValue.get()} kit.", NotifyType.INFO,3500))
                    debug("finished")
                    return
                }
            }
