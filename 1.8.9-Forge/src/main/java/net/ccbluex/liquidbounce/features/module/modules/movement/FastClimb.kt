    ...
    fun onMove(event: MoveEvent) {
        ...
        when {
            ...
            mode.equals("AAC3.0.5", ignoreCase = true) && mc.gameSettings.keyBindForward.isKeyDown &&
                    collideBlockIntersects(mc.thePlayer.entityBoundingBox, object :
                        BlockUtils.Collidable { override fun collideBlock(block: Block?) = block is BlockLadder || block is BlockVine }) -> {
                event.x = 0.0
                event.y = 0.5
                event.z = 0.0
