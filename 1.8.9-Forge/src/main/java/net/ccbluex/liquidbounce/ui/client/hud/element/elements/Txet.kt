...
class Text(x: Double = 10.0, y: Double = 10.0, scale: Float = 1F,
    ...
    private var fontValue = FontValue("Font", Fonts.SFUI40)
    ...
    override fun drawElement(): Border? {
        ...
        if (lineValue.get()) {
            ...
            for (i in 0..(gradientAmountValue.get()-1)) {
                ...
                RenderUtils.drawGradientSideways(-2.0 + barStart, -3.0, -2.0 + barEnd, -2.0,Color(ClickGUI.colorRedValue.get(), ClickGUI.colorGreenValue.get(), ClickGUI.colorBlueValue.get()).rgb,Color(ClickGUI.colorRedValue.get(), ClickGUI.colorGreenValue.get(), ClickGUI.colorBlueValue.get()).rgb)
            }
        }

        fontRenderer.drawString(displayText, 0F, 2F, when (rainbowType) {
